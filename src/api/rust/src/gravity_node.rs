...
use crate::{
    ffi, future_response::*, gravity_data_product::*, gravity_requestor::*,
    gravity_service_provider::*, gravity_subscriber::*,
};
...
pub struct GravityNode {
    gn: UniquePtr<GNode>,
    subscribers_list: Vec<(Arc<SubscriberWrap>, UniquePtr<RustSubscriber>)>,
    ...
    requestor_list: Vec<(Arc<RequestorWrap>, UniquePtr<RustRequestor>)>,
    ...
    monitor_list: Vec<(Arc<MonitorWrap>, UniquePtr<RustSubscriptionMonitor>)>,
}
...
impl GravityNode {
    ...
    pub fn unsubscribe(
        ...
        for (arcd, cpp_sub) in self.subscribers_list.iter() {
            let other = &arcd.subscriber;
            if Arc::ptr_eq(other, &subscriber) {
                item = Some(cpp_sub);
            }
        }
        match item {
            ...
            Some(sub) => {
                let_cxx_string!(dpid = data_product_id);
                ffi::unsubscribe(&self.gn, &dpid, sub)
            }
        }
    }
    ...
    pub fn publish_with_filter_text(
        &self,
        data_product: &GravityDataProduct,
        filter_text: &str,
    ) -> GravityReturnCode {
    ...
    pub fn request_async(
        &mut self,
        service_id: &str,
        request: &GravityDataProduct,
        requestor:Arc<Mutex<dyn GravityRequestor>> ,
    ) -> GravityReturnCode {

        let mut item = None;
        for (arcd, cpp_req) in self.requestor_list.iter() {
            let this = &arcd.requestor;
            if Arc::ptr_eq(this, &requestor) {
                item = Some(cpp_req);
            }
        }
        ...
        match item {
            None => {
                let mut wrap = Arc::new(RequestorWrap { requestor: requestor});
                let cpp_req = unsafe {
                    ffi::new_rust_requestor(
                        GravityNode::request_filled_internal,
                        GravityNode::request_timeout_internal,
                ...
                let ret = ffi::request_async(&self.gn,
                     &sid,
                     &request.gdp,
                     &cpp_req);
                self.requestor_list.push((wrap, cpp_req));
                ret
            }
            Some(cpp_req) => ffi::request_async(&self.gn, &sid, &request.gdp, cpp_req),
        }
    ...
    pub fn request_async_with_request_id(
        ...
        match item {
            None => {
                ...
                let ret = ffi::request_async_request_id(&self.gn,
                     &sid,
                     &request.gdp,
                     &cpp_req,
                    &rid);
            ...
            Some(cpp_req) => ffi::request_async_request_id(&self.gn, &sid, &request.gdp, cpp_req, &rid),
        }
    }
    ...
    pub fn request_async_with_timeout(
        ...
        match item {
            None => {
                ...
                let ret = ffi::request_async_timeout(&self.gn,
                     &sid,
                     &request.gdp,
                     &cpp_req,
                    &rid,
            ...
            Some(cpp_req) => ffi::request_async_timeout(&self.gn, &sid, &request.gdp, cpp_req, &rid, timeout_milliseconds),
        }
    }
    ...
    pub fn request_async_with_domain(
        ...
        match item {
            None => {
                ...
                let ret = ffi::request_async_domain(&self.gn,
                     &sid,
                     &request.gdp,
                     &cpp_req,
                    &rid,
            ...
            Some(cpp_req) => ffi::request_async_domain(&self.gn, &sid, &request.gdp, cpp_req, &rid, timeout_milliseconds, &d),
        }
    }
    ...
    /// Returns None upon failure, Some(GravityDataProduct), where the GravityDataProduct is the data product
    /// representation of the response.
    pub fn request_sync_with_timeout(
        &self,
        service_id: &str,
        request: &GravityDataProduct,
        timeout_milliseconds: i32,
    ...
    /// Returns None upon failure, Some(GravityDataProduct), where the GravityDataProduct is the data product
    /// representation of the response.
    pub fn request_sync_with_domain(
        &self,
        service_id: &str,
        request: &GravityDataProduct,
        timeout_milliseconds: i32,
    ...
    pub fn register_data_product(
        &self,
        data_product_id: &str,
        transport_type: GravityTransportType,
    ) -> GravityReturnCode {
        let_cxx_string!(dpid = data_product_id);
    ...
    /// Returns success flag.
    pub fn register_data_product_with_cache(
        &self,
        data_product_id: &str,
        transport_type: GravityTransportType,
        cache_last_value: bool,
    ) -> GravityReturnCode {
        let_cxx_string!(dpid = data_product_id);
    ...
    pub fn register_service(
        ...
        service_id: &str,
        transport_type: GravityTransportType,
        server: impl GravityServiceProvider + 'static,
    ) -> GravityReturnCode {
        let_cxx_string!(sid = service_id);
    ...
    pub fn register_heartbeat_listener(
        &mut self,
        component_id: &str,
        interval_in_microseconds: i64,
        listener: impl GravityHeartbeatListener + 'static,
    ) -> GravityReturnCode {
        let_cxx_string!(cid = component_id);
        ...
        self.listener_map.insert(
            (component_id.to_string(), "".to_string()),
            (wrap, cpp_listener),
        );
        ret
    }
    ...
    pub fn register_heartbeat_listener_with_domain(
        ...
        self.listener_map.insert(
            (component_id.to_string(), domain.to_string()),
            (wrap, cpp_listener),
        );
        ret
    }
    ...
    pub fn unregister_heartbeat_listener(&mut self, component_id: &str) -> GravityReturnCode {
        ...
        self.listener_map
            .remove(&(component_id.to_string(), "".to_string()));
        ret
    }
    ...
    pub fn unregister_heartbeat_listener_with_domain(
        ...
        self.listener_map
            .remove(&(component_id.to_string(), domain.to_string()));
        ret
    }
    ...
    pub fn code_string(&self, code: GravityReturnCode) -> String {
        ffi::get_code_string(&self.gn, code)
            .to_str()
            .unwrap()
            .to_string()
    }
    ...
    /// Returns success flag.
    pub fn clear_subscription_timeout_monitor(
        &self,
        data_product_id: &str,
        monitor:Arc<Mutex<dyn GravitySubscriptionMonitor>> ,
    ) -> GravityReturnCode {
        ...
        match item {
            ...
            Some(m) => {
                let_cxx_string!(dpid = data_product_id);
                ffi::clear_subscription_timeout_monitor(&self.gn, &dpid, m)
            }
        }
    }
    ...
    /// With parameter filter.
    /// Returns success flag.
    pub fn clear_subscription_timeout_monitor_with_filter(
        &self,
        data_product_id: &str,
        monitor:Arc<Mutex<dyn GravitySubscriptionMonitor>> ,
        filter: &str,
        ...
        match item {
            ...
            Some(m) => {
                let_cxx_string!(dpid = data_product_id);
                let_cxx_string!(f = filter);
                ffi::clear_subscription_timeout_monitor_filter(&self.gn, &dpid, m, &f)
            }
        }
    }
    ...
    /// Returns success flag.
    pub fn clear_subscription_timeout_monitor_with_domain(
        &self,
        data_product_id: &str,
        monitor:Arc<Mutex<dyn GravitySubscriptionMonitor>> ,
        filter: &str,
        ...
        match item {
            ...
            Some(m) => {
                ...
                let_cxx_string!(f = filter);
                let_cxx_string!(d = domain);
                ffi::clear_subscription_timeout_monitor_domain(&self.gn, &dpid, m, &f, &d)
            }
        }
    }
    ...
    fn sub_filled_internal(data_products: &CxxVector<GDataProduct>, ptr: *mut SubscriberWrap) {
        ...
        let next = unsafe { &mut (*ptr) };
        let subscriber = next.subscriber.clone();
        let mut s = subscriber.lock().unwrap();
        (*s).subscription_filled(v);
    }

    fn request_filled_internal(
        service_id: &CxxString,
        request_id: &CxxString,
        response: &GDataProduct,
        ...
        let mut r = requestor.lock().unwrap();
        (*r).request_filled(sid, rid, &gdp);
    }

    fn request_timeout_internal(
        service_id: &CxxString,
        request_id: &CxxString,
        ptr: *mut RequestorWrap,
    ) {
        ...
        let requestor = &mut next.requestor;
        let mut r = requestor.lock().unwrap();
        (*r).request_timeout(sid, rid);
    }

    fn request_internal(
        service_id: &CxxString,
        data_product: &GDataProduct,
        ptr: *mut ServiceWrap,
    ) -> SharedPtr<GDataProduct> {
    ...
    fn missed_heartbeat_internal(
        component_id: &CxxString,
        microsecond_to_last_heartbeat: i64,
        interval_in_microseconds: &mut i64,
        ptr: *mut ListenerWrap,
    ) {
    ...
    fn received_heartbeat_internal(
        component_id: &CxxString,
        interval_in_microseconds: &mut i64,
        ptr: *mut ListenerWrap,
    ) {
    ...
    fn subscription_timeout_internal(
        data_product_id: &CxxString,
        milli_seconds_since_last: i32,
        filter: &CxxString,
        domain: &CxxString,
        ptr: *mut MonitorWrap,
    ) {
        ...
        let next = unsafe { &mut (*ptr) };
        let monitor = next.monitor.clone();
        let mut m = monitor.lock().unwrap();
        (*m).subscription_timeout(dpid, milli_seconds_since_last, f, d);
    }
}
